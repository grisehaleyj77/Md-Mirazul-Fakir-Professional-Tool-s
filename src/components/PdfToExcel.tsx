...
import { FileText, FileDown, Upload, X, FileSpreadsheet } from 'lucide-react';
  ...
  const convertToExcel = async () => {
    ...
    try {
      ...
      for (let i = 1; i <= pdf.numPages; i++) {
        ...
        // Group items by roughly the same Y coordinate to form rows
        const rowsMap = new Map<number, { x: number, text: string }[]>();
        
        for (const item of textContent.items) {
          if ('str' in item && item.str.trim()) {
            // @ts-ignore
            const y = Math.round(item.transform[5] / 5) * 5; // Group by 5px tolerance
            // @ts-ignore
            ...
            rowsMap.get(y)?.push({ x, text: item.str });
          }
        }
        ...
        for (const y of sortedY) {
          ...
          // Basic heuristic to separate columns based on X distance
          const row: string[] = [];
          let lastX = -1;
          
          for (const item of rowItems) {
            if (lastX !== -1 && item.x - lastX > 50) {
              // Large gap, assume new column
              row.push(item.text);
            } else if (row.length > 0) {
              // Small gap, append to last column
              row[row.length - 1] += ' ' + item.text;
            } else {
              row.push(item.text);
            }
    ...
    <div className="flex flex-col gap-8">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-neutral-500 text-lg">
          Upload a PDF file to extract its text and tables into a Microsoft Excel (.xlsx) document.
        </p>
        <p className="text-sm text-neutral-400 italic">
          Note: This performs basic text extraction and attempts to guess columns based on spacing. Complex tables may not be perfectly preserved.
        </p>
      </div>
