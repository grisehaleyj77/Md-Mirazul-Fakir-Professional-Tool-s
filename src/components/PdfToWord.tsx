  ...
  const convertToWord = async () => {
    ...
    try {
      ...
      for (let i = 1; i <= pdf.numPages; i++) {
        ...
        // Group items by roughly the same Y coordinate to form lines/paragraphs
        let currentY = -1;
        let currentLine = '';
        
        for (const item of textContent.items) {
          if ('str' in item) {
            // @ts-ignore - transform is an array of 6 numbers
    ...
    <div className="flex flex-col gap-8">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-neutral-500 text-lg">
          Upload a PDF file to extract its text into a Microsoft Word (.docx) document.
        </p>
        <p className="text-sm text-neutral-400 italic">
          Note: This performs basic text extraction. Complex formatting, images, and tables may not be preserved.
        </p>
      </div>
